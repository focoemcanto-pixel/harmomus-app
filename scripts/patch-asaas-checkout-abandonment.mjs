...
const scheduleFrom = `function scheduledAtForAutomation(automation: AutomationRow) {\n  const delay = delayMinutesForAutomation(automation);\n  return delay > 0 ? addMinutes(delay) : null;\n}`;
const scheduleTo = `function scheduledAtForAutomation(automation: AutomationRow, event?: MarketingEventRow | null) {\n  if (event?.metadata?.abandonment_delay_already_elapsed === true) return null;\n  const delay = delayMinutesForAutomation(automation);\n  return delay > 0 ? addMinutes(delay) : null;\n}`;
if (!source.includes(scheduleTo)) {
  if (!source.includes(scheduleFrom)) throw new Error("Função de agendamento não encontrada");
  source = source.replace(scheduleFrom, scheduleTo);
}

const scheduleCallFrom = `      const scheduledAt = scheduledAtForAutomation(winner.automation);`;
const scheduleCallTo = `      const scheduledAt = scheduledAtForAutomation(winner.automation, latestEvent);`;
if (!source.includes(scheduleCallTo)) {
  if (!source.includes(scheduleCallFrom)) throw new Error("Chamada de agendamento não encontrada");
  source = source.replace(scheduleCallFrom, scheduleCallTo);
}
