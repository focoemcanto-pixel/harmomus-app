...
function patchFile(relativePath, patches, label) {
  ...
  for (const patch of patches) {
    // Alguns patches inserem blocos antes de um trecho que continua existindo no
    // resultado final. Nesses casos, comparar apenas `to` não é suficiente para
    // detectar uma execução anterior. Um marcador estável impede nova inserção.
    if (patch.appliedWhen && source.includes(patch.appliedWhen)) continue;
...
patchFile(
  ...
  [
    ...
    },
    {
      ...
      appliedWhen: `const existingGateway = normalizeSubscriptionValue(existing?.gateway);`,
  ...
  [
    {
      ...
      appliedWhen: `const activeOtherGateway = rows.find((subscription) => {`,
  ...
  [
    {
      ...
      appliedWhen: `async function recordStripeCrossGatewayConflict(`,
    ...
    },
    {
      ...
      appliedWhen: `await recordStripeCrossGatewayConflict(supabase, event, userId, previous, syncedSubscriptionId);`,
  ...
  [
    {
      ...
      appliedWhen: `async function findActiveOtherGatewaySubscription(`,
    ...
    },
    {
      ...
      appliedWhen: `reason: "active_subscription_owned_by_other_gateway"`,
