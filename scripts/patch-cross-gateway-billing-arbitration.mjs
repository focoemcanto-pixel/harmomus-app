...
function hasAppliedMarker(source, patch) {
  const markers = Array.isArray(patch.applied)
    ? patch.applied
    : [patch.applied ?? patch.to];
...
function patchFile(relativePath, patches, label) {
  ...
  for (const patch of patches) {
    if (hasAppliedMarker(source, patch)) {
      console.log(`[${label}] Já aplicado: ${patch.name}`);
      continue;
    }

    if (!source.includes(patch.from)) {
      throw new Error(
        `[${label}] Trecho não encontrado e marcador de aplicação ausente: ${patch.name}`,
      );
    }
  ...
  [
    {
      ...
      applied: `import { cancelSubscription as cancelAsaasSubscription } from "@/lib/asaas/subscriptions";`,
    ...
    },
    {
      ...
      applied: `.select("id,status,gateway,gateway_subscription_id,current_period_end`,
    ...
    },
    {
      ...
      applied: `existingGateway && existingGateway !== "stripe"`,
  ...
  [
    {
      ...
      applied: `const activeOtherGateway = rows.find((subscription) => {`,
  ...
  [
    {
      ...
      applied: `async function recordStripeCrossGatewayConflict(`,
    ...
    },
    {
      ...
      applied: `await recordStripeCrossGatewayConflict(supabase, event, userId, previous, syncedSubscriptionId);`,
    ...
    },
    {
      ...
      applied: `if (context) await saveBillingInvoiceFromStripeEvent(supabase, event, context);`,
  ...
  [
    {
      ...
      applied: [
        `async function findActiveOtherGatewaySubscription(`,
        `async function recordAsaasCrossGatewayConflict(`,
      ],
    ...
    },
    {
      ...
      applied: `return NextResponse.json({ received: true, synced: false, ignored: true, reason: "active_subscription_owned_by_other_gateway" });`,
