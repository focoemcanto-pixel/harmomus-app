...
function UserSilhouetteIcon() {
  return (
    ...
    >
      <path
        fill="currentColor"
        d="M12 12c2.76 0 5-2.24 5-5s-2.24-5-5-5 2.24-5 5 2.24 5 5 5 5Zm0 2c-3.87 0-8 2.03-8 5v1c0 .55.45 1 1 1h14c.55 0 1-.45 1-1v-1c0-2.97-4.13-5-8-5Z"
      />
    </svg>
  );
}
...
export function PublicShellClient({
  ...
  context: CurrentUserAccessContext;
  ...
  const [liveAvatar, setLiveAvatar] = useState(
    context.isGuest ? null : (context.profile?.avatar_url ?? null),
  );
  ...
  const planSlug = String(context.plan?.slug ?? "")
    .trim()
    .toLowerCase();
  const isMinistryPlan = MINISTRY_PLAN_SLUGS.has(planSlug);
  const isMinistryUser = Boolean(context.ministry) || isMinistryPlan;
  const ministryRole = String(context.ministry?.role ?? "")
    .trim()
    .toLowerCase();
  const canOpenMinistryCentral = context.ministry
    ? ministryRole === "owner" ||
      ministryRole === "admin" ||
  ...
  useEffect(() => {
    function onClickOutside(ev: MouseEvent) {
      ...
      const clickedDesktop =
        searchDesktopRef.current?.contains(ev.target as Node) ?? false;
      const clickedMobile =
        searchMobileRef.current?.contains(ev.target as Node) ?? false;
  ...
  const results = useMemo(() => {
    if (!debounced) return [];
    return searchItems
      .filter((item) => item.searchText.includes(debounced))
      .slice(0, 8);
  }, [debounced, searchItems]);

  function onProtectedClick(type: "plus" | "premium") {
    const allowedPlus =
      context.effectiveSlug === "plus" || context.effectiveSlug === "premium";
    const allowedPremium = context.effectiveSlug === "premium";
    if (
      (type === "plus" && !allowedPlus) ||
      (type === "premium" && !allowedPremium)
    ) {
      const isPlus = type === "plus";
      setUpgradeConfig({
        title: isPlus
          ? "Este recurso requer plano Plus ou Premium."
          : "Este recurso requer plano Premium.",
        message: isPlus
  ...
  return (
    <>
      <div
        className="min-w-0 max-w-[58vw] flex-1 md:hidden"
        ref={searchMobileRef}
      >
        <div className="relative">
          <input
            ...
            className="h-8 w-full rounded-lg border border-white/15 bg-white/5 px-3 text-xs text-white outline-none ring-cyan-300/40 transition placeholder:text-zinc-400 focus:ring"
          />
          {results.length > 0 ? (
            <div className="absolute left-0 right-0 top-11 z-50 rounded-xl border border-white/10 bg-[#0d1220] p-2 shadow-premium">
              {results.map((item) => (
                <Link
                  key={item.id}
      ...
      <div
        className="relative mx-1 hidden flex-1 md:block"
        ref={searchDesktopRef}
      >
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Buscar kits, artista ou categoria"
          className="h-11 w-full rounded-xl border border-white/15 bg-white/5 px-4 text-sm outline-none ring-cyan-300/40 transition focus:ring"
        />
        {results.length > 0 ? (
      ...
      <Link
        href="/todos-os-kits"
        className="hidden whitespace-nowrap rounded-lg border border-white/20 px-3 py-2 text-sm text-zinc-100 md:inline-flex"
      >
        Todos os Kits
      </Link>
      <div className="relative" ref={menuRef}>
        <button
          ...
          className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-full border border-white/20 bg-white/5 text-xs font-semibold md:h-9 md:w-9"
        >
          {showAvatar ? (
            <img
              src={liveAvatar!}
        ...
        {menuOpen ? (
          <div className="absolute right-0 top-11 z-50 min-w-60 rounded-xl border border-white/10 bg-[#0d1220] p-2 shadow-premium">
            {context.isGuest ? (
              ...
              <>
                {isMinistryUser ? (
                  <div className="mb-2 rounded-xl border border-cyan-300/20 bg-cyan-400/10 px-3 py-2 text-xs text-cyan-100">
                ...
                <Link
                  ...
                  onClick={(e) => {
                    if (!onProtectedClick("plus")) e.preventDefault();
                  }}
                  className="block rounded-lg px-3 py-2 text-sm hover:bg-white/5"
                >
                  Minhas Playlists
                ...
                <Link
                  href="/area-premium"
                  onClick={(e) => {
                    if (!onProtectedClick("premium")) e.preventDefault();
                  }}
                  className="block rounded-lg px-3 py-2 text-sm hover:bg-white/5"
                >
                  Área Premium
                ...
                <Link
                  href="/logout"
                  className="block rounded-lg px-3 py-2 text-sm text-rose-300 hover:bg-white/5"
                >
                  Sair
                </Link>
              </>
