...
export function TemplateManager() {
  ...
  const [name, setName] = useState("Boas-vindas Harmomus");
  const [channel, setChannel] = useState<"whatsapp" | "email" | "both">("both");
  const [category, setCategory] = useState("campanha");
  const [subject, setSubject] = useState("Novidade no Harmomus");
  const [body, setBody] = useState("Olá {{nome}}, temos uma novidade para você: {{link}}");
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
      <section className="rounded-3xl border border-white/10 bg-slate-950/70 p-5">
        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="text-sm text-slate-300">Categoria<input value={category} onChange={(e) => setCategory(e.target.value)} className="mt-2 w-full rounded-2xl border border-white/10 bg-slate-900 px-4 py-3 text-white" /></label>
          </div>
      ...
      <section className="rounded-3xl border border-white/10 bg-slate-950/70 p-5">
        <h3 className="text-lg font-semibold text-white">Templates salvos</h3>
