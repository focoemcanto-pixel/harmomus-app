...
export function WebhookCenter() {
  ...
  const [openCats, setOpenCats] = useState<Record<string, boolean>>({ ASSINATURAS: true, CHECKOUT: true, PAGAMENTOS: true, USUÁRIOS: false, PLATAFORMA: false });
  ...
  async function openTest(endpoint: WebhookEndpoint) {
    ...
    const res = await fetch("/api/admin/webhooks/test", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ endpointId: endpoint.id, event: defaultEvent, previewOnly: true }),
    });
    const json = await res.json();
  ...
  async function refreshPreview() {
    ...
    const res = await fetch("/api/admin/webhooks/test", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ endpointId: testingEndpoint.id, event: testEvent, previewOnly: true }),
    });
    const json = await res.json();
    setTestPreview(json.payload ?? null);
    setTestHeaders(json.headers ?? null);
  ...
  async function sendTest() {
    ...
    const res = await fetch("/api/admin/webhooks/test", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ endpointId: testingEndpoint.id, event: testEvent }),
    });
    const json = await res.json();
    if (!res.ok) {
