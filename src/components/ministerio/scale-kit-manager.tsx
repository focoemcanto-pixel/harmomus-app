...
export function ScaleKitManager({ repertoireId, selectedSongs = [] }: ScaleKitManagerProps) {
  ...
  return (
    <div className="mt-5">
      ...
      {modalOpen ? (
        <div className="fixed inset-0 z-[9999] isolate overflow-y-auto bg-black/80 p-4 backdrop-blur-md md:p-8">
          <button type="button" aria-label="Fechar montagem" onClick={closeModal} className="fixed inset-0 z-0 cursor-default" />
          <div className="relative z-10 mx-auto flex min-h-full w-full max-w-5xl items-center">
            <div className="relative w-full rounded-[2rem] border border-white/10 bg-[#090d18] p-5 shadow-2xl shadow-black/60 md:p-7" onClick={(event) => event.stopPropagation()}>
