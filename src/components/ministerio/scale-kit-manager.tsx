...
import { Check, Library, Loader2, Plus, Search, X } from "lucide-react";
...
export function ScaleKitManager({ repertoireId }: ScaleKitManagerProps) {
  ...
  const endpoint = useMemo(
    () => `/api/ministerio/repertorios/${repertoireId}/kits`,
    [repertoireId],
  );
  const cleanQuery = query.trim();

  function applySearchPayload(
    payload: { kits?: Kit[]; existingKitIds?: string[] },
    target: "search" | "library",
  ) {
  ...
  function loadKits(
    ...
    const transition =
      target === "library" ? startLibraryLoading : startLoading;
    ...
    try {
      ...
      setMessage(
        payload?.alreadyAdded
          ? "Esse kit já estava na escala."
          : `${kit.name} foi adicionado à escala.`,
      );
    } finally {
      setAddingKitId(null);
    }
  ...
  function KitRow({ kit, compact = false }: { kit: Kit; compact?: boolean }) {
    ...
    return (
      <div
        className={`flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[0.035] ${compact ? "p-2" : "p-3"}`}
      >
        <button
          type="button"
          onClick={() => addKit(kit)}
        ...
        >
          <div
            className={`${compact ? "h-9 w-9" : "h-12 w-12"} shrink-0 overflow-hidden rounded-xl bg-white/5`}
          >
            {kit.cover_url ? (
              <img
                src={kit.cover_url}
        ...
        <div className="flex shrink-0 items-center gap-2">
          {!compact ? (
            <Link
              ...
              className="rounded-xl border border-white/10 px-3 py-2 text-xs font-semibold text-zinc-200 transition hover:bg-white/10"
            >
              Ver kit
            </Link>
          ) : null}
    ...
    <div className="mt-5 space-y-3">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="relative md:max-w-xl md:flex-1">
          ...
          {hasSearched ? (
            <div className="absolute z-30 mt-2 max-h-80 w-full overflow-y-auto rounded-3xl border border-white/10 bg-[#090d18] p-2 shadow-2xl shadow-black/40">
              ...
              {!loading &&
                kits.map((kit) => <KitRow key={kit.id} kit={kit} compact />)}
              {!loading && !kits.length ? (
        ...
        <button
          type="button"
          onClick={openLibrary}
          className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-5 py-3 text-sm font-semibold text-zinc-200 transition hover:bg-white/10"
        >
        ...
        <div className="fixed inset-0 z-50">
          ...
          <aside className="absolute right-0 top-0 flex h-full w-full max-w-xl flex-col border-l border-white/10 bg-[#090d18] p-5 shadow-2xl shadow-black/50">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-400">
                  Navegue pelo acervo sem poluir a tela principal da escala.
                </p>
              </div>
              <button
                type="button"
                onClick={() => setDrawerOpen(false)}
                className="rounded-2xl border border-white/10 p-3 text-zinc-300 transition hover:bg-white/10"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="mt-5 flex-1 overflow-y-auto pr-1">
              {libraryLoading ? (
                <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.035] p-4 text-sm text-zinc-300">
