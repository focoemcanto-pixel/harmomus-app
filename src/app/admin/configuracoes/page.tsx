...
export default async function ConfiguracoesPage() {
  ...
  async function save(formData: FormData) {
    ...
    await saveAdminSettings({
      branding: { appName: String(formData.get("appName") ?? ""), logoUrl: String(formData.get("logoUrl") ?? ""), faviconUrl: String(formData.get("faviconUrl") ?? ""), primaryColor: String(formData.get("primaryColor") ?? "#D4AF37") },
  ...
  <form action={save} className="rounded-xl border border-border bg-surface p-6 shadow-premium grid gap-3 md:grid-cols-2 text-sm">
    ...
    <input name="logoUrl" defaultValue={settings.branding.logoUrl} className="rounded border border-border bg-background px-3 py-2" placeholder="Logo URL" />
