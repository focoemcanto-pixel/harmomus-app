...
function statusClass(status?: string | null) {
  ...
  if (label === "Ativo") return "border-emerald-400/25 bg-emerald-500/10 text-emerald-200";
...
async function getSubscriptionCenterData() {
  ...
  return {
    subscriptions,
    invoices,
    subscriptionError: subscriptionsResult.error?.message ?? null,
    invoiceError: invoicesResult.error?.message ?? null,
  };
}

export default async function AdminSubscriptionsPage() {
  const { subscriptions, invoices, subscriptionError, invoiceError } = await getSubscriptionCenterData();

  const active = subscriptions.filter((row) => isActive(row.status));
  const pending = subscriptions.filter((row) => normalize(row.status) === "pending");
  ...
  const planStats = [
    ...
    { label: "Pendente", count: pending.length, tone: "bg-amber-400" },
  ];
  const maxPlanCount = Math.max(...planStats.map((item) => item.count), 1);
  const recentSubscriptions = subscriptions.slice(0, 30);
