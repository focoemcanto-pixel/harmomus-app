...
export default function WebhookEndpointDetailsPage() {
  ...
  useEffect(() => { void (async () => { if (!params?.id) return; const res = await fetch(`/api/admin/webhooks/endpoints/${params.id}`); const j = await res.json(); setEndpoint(j.data); setLogs(j.logs ?? []); setEvent(j?.data?.events?.[0] ?? ""); })(); }, [params]);
