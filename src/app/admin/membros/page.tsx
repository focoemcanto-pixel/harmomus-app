  ...
  return (
    <section className="space-y-6">
      ...
      <div className="rounded-xl border border-border bg-surface shadow-premium overflow-x-auto">
        <table className="w-full min-w-[980px] text-sm"><thead className="text-left text-muted"><tr><th className="p-4">Membro</th><th>Plano</th><th>Status</th><th>Renovação</th><th>Gateway</th><th>Stripe Customer</th><th>Stripe Sub</th><th>Próx. cobrança</th><th></th></tr></thead>
          <tbody>{members.map((m)=><tr key={m.profile.id} className="border-t border-border/70"><td className="p-4"><p className="font-medium">{m.profile.full_name ?? "Sem nome"}</p><p className="text-xs text-muted">{m.profile.email ?? "Sem e-mail"}</p></td><td>{m.plan?.name ?? "Sem plano"}</td><td><span className="rounded-full bg-surface-muted px-3 py-1 text-xs">{m.subscription?.status ?? "none"}</span></td><td>{m.subscription?.current_period_end ? formatDateTimeBR(m.subscription.current_period_end) : "-"}</td><td>{m.subscription?.gateway ?? "-"}</td><td>{m.subscription?.stripe_customer_id ?? "-"}</td><td>{m.subscription?.stripe_subscription_id ?? "-"}</td><td>{m.subscription?.next_billing_at ? formatDateTimeBR(m.subscription.next_billing_at) : "-"}</td><td><Link href={`/admin/membros/${m.profile.id}`} className="text-gold-300">Detalhes</Link></td></tr>)}</tbody>
        </table>
      </div>
    </section>
  );
