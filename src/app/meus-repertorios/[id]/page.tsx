...
export default async function MeuRepertorioDetalhePage({ params }: { params: Promise<PageParams> }) {
  ...
  const [{ data: ministry }, { data: repertoire, error }] = await Promise.all([admin.from("ministries").select("id,name").eq("id", context.ministry.ministryId).maybeSingle(), admin.from("ministry_repertoires").select("id,name,description,event_date,created_at,archived,ministry_id,coordinator_member_id").eq("id", resolvedParams.id).eq("ministry_id", context.ministry.ministryId).maybeSingle()]);
  if (error) throw new Error(error.message);
