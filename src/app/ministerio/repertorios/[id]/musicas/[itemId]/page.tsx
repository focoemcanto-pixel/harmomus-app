...
type MinistryMemberRow = { id: string; invited_name: string | null; invited_email: string | null; status?: string | null };
type AssignmentRow = { member_id: string; assigned_voice: string | null; notes: string | null };

function memberLabel(member: MinistryMemberRow) { return member.invited_name || member.invited_email || "Integrante"; }
...
export default async function SongSettingsPage({ params, searchParams }: { params: Promise<PageParams>; searchParams?: Promise<PageSearchParams> }) {
  ...
  const activeMembers = ((members ?? []) as MinistryMemberRow[]).filter((member) => member.status !== "removed");
  const membersById = new Map<string, MinistryMemberRow>(activeMembers.map((member) => [member.id, member]));
  const songMap = new Map<string, AssignmentRow>(((songAssignments ?? []) as AssignmentRow[]).map((assignment) => [assignment.member_id, assignment]));
  const rows = ((scaleAssignments ?? []) as AssignmentRow[]).map((assignment) => { const member = membersById.get(assignment.member_id); const song = songMap.get(assignment.member_id); return member ? { memberId: member.id, name: memberLabel(member), defaultVoice: assignment.assigned_voice, defaultNotes: assignment.notes, songVoice: song?.assigned_voice ?? null, songNotes: song?.notes ?? null } : null; }).filter((row): row is NonNullable<typeof row> => Boolean(row));
  const kit = Array.isArray(item.kits) ? item.kits[0] : item.kits;
