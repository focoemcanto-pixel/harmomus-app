...
async function createScale(formData: FormData) {
  ...
  if (teamTemplateId) {
    const { data: template } = await admin.from("ministry_team_templates").select("id,coordinator_member_id").eq("id", teamTemplateId).eq("ministry_id", context.ministry.ministryId).eq("archived", false).maybeSingle();
    if (template?.id) {
      resolvedCoordinatorId = resolvedCoordinatorId || template.coordinator_member_id || null;
      const { data: members } = await admin.from("ministry_team_template_members").select("member_id,assigned_voice,notes").eq("template_id", template.id);
      templateMembers = members ?? [];
    }
  }

  const finalDescription = [description, generalNotes ? `Observação geral: ${generalNotes}` : ""].filter(Boolean).join("\n\n") || null;
  const { data, error } = await admin.from("ministry_repertoires").insert({ ministry_id: context.ministry.ministryId, name, description: finalDescription, event_date: eventDate || null, team_template_id: teamTemplateId || null, coordinator_member_id: resolvedCoordinatorId, status: "scheduled", created_by: context.profile?.id ?? null, archived: false, created_at: now, updated_at: now }).select("id").single();
  ...
  await logMinistryActivity({ ministryId: context.ministry.ministryId, actorUserId: context.profile?.id ?? null, actorName, action: "scale.created", entityType: "ministry_repertoire", entityId: data.id, description: `${actorName} criou a escala ${name}`, metadata: { repertoire_id: data.id, name, description: finalDescription, event_date: eventDate || null } });
