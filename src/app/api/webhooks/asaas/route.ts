...
async function markEvent(supabase: ReturnType<typeof createSupabaseAdminClient>, payload: AsaasWebhookPayload, processed: boolean, errorMessage?: string, extra: Record<string, unknown> = {}) {
  ...
  if (error) {
    ...
    throw new Error(`Falha ao registrar billing_event: ${error.message}`);
  }
}
...
async function getCheckoutPlan(supabase: ReturnType<typeof createSupabaseAdminClient>, subscriptionId: string | null) {
  ...
  return { planSlug: normalizeLower(payload?.plan_slug), previousPlanSlug: normalizeLower(payload?.previous_plan_slug), userId: String(payload?.user_id ?? "").trim() || null };
}
...
async function saveBillingInvoiceFromAsaasEvent(input: {
  ...
  if (!payment?.id || !["PAYMENT_CREATED", "PAYMENT_RECEIVED", "PAYMENT_CONFIRMED", "PAYMENT_OVERDUE"].includes(input.event)) return;

  const paymentStatus = normalizeLower(payment.status);
  const isPaid = isConfirmedPaymentEvent(input.event, input.payload);
  const isOverdue = isPaymentOverdueEvent(input.event, input.payload);
  const amountCents = cents(payment.value);
  const paidAt = isPaid ? parseAsaasDate(payment.paymentDate ?? payment.clientPaymentDate ?? payment.confirmedDate ?? null) : null;
  const periodStart = parseAsaasDate(payment.paymentDate ?? payment.clientPaymentDate ?? payment.dateCreated ?? input.payload.dateCreated ?? null);
  const providerEventId = asaasExternalEventId(input.payload);
  const status = isPaid ? "paid" : isOverdue ? "overdue" : paymentStatus === "pending" ? "open" : paymentStatus ?? "open";

  const { data: profile, error: profileError } = await input.supabase.from("profiles").select("email").eq("id", input.subscription.user_id).maybeSingle();
  if (profileError) console.error("[asaas.webhook] Falha ao buscar e-mail para billing_invoice", profileError);
  ...
  const invoicePayload = {
    ...
    plan_id: input.subscription.plan_id ?? null,
    ...
    amount_paid_cents: isPaid ? amountCents : 0,
    amount_remaining_cents: isPaid ? 0 : amountCents,
    invoice_url: payment.transactionReceiptUrl ?? payment.invoiceUrl ?? null,
    hosted_invoice_url: payment.invoiceUrl ?? null,
    paid_at: paidAt,
    period_start: periodStart,
    period_end: input.periodEnd ?? input.subscription.current_period_end ?? null,
    raw_payload: payment,
    updated_at: new Date().toISOString(),
  };

  const { error } = await input.supabase.from("billing_invoices").upsert(invoicePayload, { onConflict: "provider,provider_invoice_id" });
  if (error) console.error("[asaas.webhook] Falha ao salvar billing_invoice Asaas", error);
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const syncedSubscription: SubscriptionRow = { ...fallbackSubscription, plan_id: (updatePayload.plan_id as string | undefined) ?? fallbackSubscription.plan_id, current_period_end: (updatePayload.current_period_end as string | undefined) ?? fallbackSubscription.current_period_end };
    await saveBillingInvoiceFromAsaasEvent({ supabase, payload, subscription: syncedSubscription, event, periodEnd });
