...
import { findCustomerByEmail, createCustomer } from "@/lib/asaas/customers";
...
function customerName(userEmail: string, profile: ProfileRow | null) {
  return cleanValue(profile?.full_name) || userEmail.split("@")[0] || "Cliente Harmomus";
}
...
export async function GET(req: Request) {
  ...
  try {
    if (!ALLOWED_PLAN_SLUGS.has(planSlug)) {
      return NextResponse.redirect(appUrl(req, "/assinar?error=Plano%20inv%C3%A1lido"), { status: 303 });
    }
    if (!ALLOWED_METHODS.has(method)) {
      return NextResponse.redirect(appUrl(req, `/checkout?plan=${encodeURIComponent(planSlug)}&error=M%C3%A9todo%20inv%C3%A1lido`), { status: 303 });
    }
    ...
    const customer = existingAsaasCustomerId
      ...
      : (await findCustomerByEmail(email)) ?? await createCustomer({
        name: customerName(email, typedProfile),
        email,
    ...
    const asaasSubscription = await createSubscription({
      ...
      description: `Harmomus ${plan.name}`,
      externalReference: user.id,
    });
