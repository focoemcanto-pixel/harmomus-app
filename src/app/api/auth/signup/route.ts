...
function buildErrorRedirect(request: Request, input: { plan: string; redirectTo: string; message: string; field: Field; fullName: string; username: string; email: string; phone: string }) {
...
export async function POST(request: Request) {
  ...
  if (!phone || phone.replace(/\D/g, "").length < 10) return fail("Informe um WhatsApp válido.", "phone");
  ...
  const next = plan === "free" ? "/cadastro/sucesso?plan=free" : `/api/billing/checkout?plan=${plan}&welcome=1`;
