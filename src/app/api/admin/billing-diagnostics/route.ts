...
export async function GET() {
  ...
  const [{ data: subscriptions, error: subscriptionsError }, { data: billingEvents, error: billingEventsError }] = await Promise.all([
    admin
      .from("subscriptions")
      .select("id,user_id,plan_id,status,gateway,stripe_customer_id,gateway_customer_id,stripe_subscription_id,gateway_subscription_id,stripe_price_id,current_period_end,next_billing_at,trial_ends_at,auto_renew,canceled_at,last_webhook_event,created_at,updated_at,plan:plans(slug,name)")
