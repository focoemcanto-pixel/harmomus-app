...
export interface KitListItem extends Kit { category_name: string | null }

export async function getKits(): Promise<KitListItem[]> {
  const supabase = (await createClient()) as any;
  const [{ data: kits, error: kitsError }, { data: categories, error: categoriesError }] = await Promise.all([
  ...
  return ((kits ?? []) as Kit[]).map((kit) => ({ ...kit, category_name: kit.category_id ? categoriesMap.get(kit.category_id) ?? null : null }));
}
